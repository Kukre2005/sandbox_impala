...
var app = angular
        .module('myapp', [
            ...
            'LocalStorageModule', 'angularModalService'
        ]).constant("constantData", (function () {
    // Define your variable
    var resource = 'http://localhost/reservations';
    ...
    return {
        ...
        .config(['cfpLoadingBarProvider', 'stConfig', '$stateProvider', function (cfpLoadingBarProvider, stConfig, $stateProvider) {
                cfpLoadingBarProvider.includeSpinner = false;

                cfpLoadingBarProvider.latencyThreshold = 1;

            }]).run(['$rootScope', '$state', '$stateParams', 'dataFactory', 'localStorageService', '$templateCache', 'ModalService', 'constantData', 'env', function ($rootScope, $state, $stateParams, dataFactory, localStorageService, $templateCache, ModalService, constantData, env) {
        $rootScope.miniPass = '0';
        $rootScope.SITE_NAME = 'Insideloscabos';
        $rootScope.SITE_URL = env.get("apiroot");
        $rootScope.itemsPerPageData = ['10', '25', '50', '100'];
        $rootScope.itemsPerPage = $rootScope.itemsPerPageSelect = constantData.ITEMS_PER_PAGE;
...
//            //console.log("fromState", fromState);
            ...
            dataFactory.httpRequest("/admin/auth/checkLogin", "POST", {}, {}).then(function (response) {
                ...
                if ((toState.name == 'app.hotels' || toState.name == 'app.groups') && login == true && $rootScope.miniPass == '0')
                {
                    $state.go("app.main");
                }
...
//   });
        $rootScope.showMsg = function (type, message, elementId)
        {
            ...
            var successDiv = '<div class="alert alert-' + a + ' errorSuccess" id="my' + elementId + '" > <a href="javascript:void(0)" class="close" data-dismiss="alert" aria-label="close" title="close">×</a>\
                        <strong>' + b + '!</strong>&nbsp;' + message + '</div>';
        ...
        $rootScope.select = function (id) {
            ...
            if (found == -1)
                ...
                $rootScope.selected.splice(found, 1);
...
//            //console.log("sel", $rootScope.selected);
        ...
        $rootScope.changeStatus = function (table, field, value, id, elemId, alert)
        {
            alert = (alert && alert != undefined) ? alert : false;
            var c = (alert == true) ? confirm("Are you sure you want to do this?") : true;
            if (c == true)
            {
                $rootScope.isLoading = true;
                var postParam = {"table": table, "field": field, "value": value, "id": id};
//                //console.log("pp", postParam);
                dataFactory.httpRequest('/admin/auth/changeStatus', 'POST', '', postParam).then(function (response) {
