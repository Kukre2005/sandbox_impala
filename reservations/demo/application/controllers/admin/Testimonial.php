...
class Testimonial extends MIK_Controller {
...
//        $start = ($page - 1) * $limit;
  ...
  public function get_image_thumb($filepath,$subfolder,$width,$height,$min_width="",$min_height="")
  {
    ...
    public function addEditTestimonial()
    {
        ...
        $original_image = $this->corefileUploading('image','testimonial');
        $info['user_original_image'] = $original_image;
        $info['user_thumbnail_image'] = $this->get_image_thumb($original_image,'testimonial',100,100);
        $info['descprition'] = @$_POST['descprition'];
        $info['user_location'] = @$_POST['user_location'];
        $info['username'] = @$_POST['username'];
        unset($info['image']);
        unset($info['type']);
        $r=0;
        $rules['username'] = array('username', 'trim|required');
        $rules['user_location'] = array('location', 'trim|required');
        $rules['descprition'] = array('descprition', 'trim|required');
