...
class Transportation extends MIK_Controller {
    ...
    public function login() {
        ...
        if ($_POST && count($_POST) > 0) {
            ...
            $c = 0;
            if (!empty($code) && $email) {
                $dbHotel = $this->Commonmodel->get_single_data("hotels", "code = '$code'", $fields = '*');
                $c =count($dbHotel);
            }
...
//                }
    ...
    public function bookPrivate() {
        ...
        if ($_POST && count($_POST) > 0) {
            ...
            $hotelId = $info['hotelId'] = $data['loginData']['hotelId'];
...
//                $message = strip_tags(validation_errors());
        ...
        if ($mstatus == 'error') {
            ...
            $data['info']['hotelId'] = $data['loginData']['hotelId'];
        }
        $data['mo'] = $mo;
...
//$info['paypalTaxAmt']=$paypalData['TAXAMT'];
            ...
            try {
                ...
                $msgBody = '<html><body>';
                $msgBody .= '<img src="' . site_url('assets/img/logo.png') . '" alt="Impala" />';
                $userMsgBody = $msgBody;
                ...
                $tableBody .= "</table><br>";
                $tableBody .= "<div>You will receive your confirmation transportation voucher from: no_reply@impala.com</div>";
                $tableBody .= '<br><div>Regards,<br><br>';
                $tableBody .= "<strong>" . SITE_NAME . "</strong><br><br>";
                $tableBody .= '<img src="' . site_url('assets/img/logo.png') . '" alt="Impala" />';
                ...
                $o = $this->Email_model->sendEmail($userTo, $usersubject, $userMsgBody, "", FROM_EMAIL, FROM_NAME);
            } catch (Exception $ex) {
    ...
    function BookingComplete() {
        ...
        if (count($dbBook) > 0) {
            $data = $dbBook;
            $data['mo'] = "paypal_result";
            $data['paypalTitle'] = "PayPal - Payment Success";
