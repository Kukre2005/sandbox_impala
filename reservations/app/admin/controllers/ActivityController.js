app.controller('ActivityController', function (env,$window, $scope, $rootScope, $http, $filter, $state, $stateParams, $location, dataFactory, localStorageService, constantData) {
    ...
    if ($state.params.actId)
    {
        ...
        actId = $state.params.actId;
        $scope.formData.actId = actId;
        if ($scope.slugData == null)
        {
            $scope.isLoading = true;
            dataFactory.httpRequest('/admin/activity/getActivity', 'POST', '', {actId: actId}).then(function (response) {
    ...
    $scope.init = function ()
    {
        ...
        var u = ($state.params.actId) ? "activity/subActivity" : "activity";
        $scope.isLoading = true;
...
//        });
    ...
    $scope.deleteActivity = function () {
        if ($scope.selected.length > 0)
        {
            if (confirm("Are you sure you want to delete?"))
            {
                ...
                dataFactory.httpRequest('/admin/activity/deleteActivity', 'POST', '', pParam).then(function (response) {
                    $scope.isLoading = false;
    ...
    $scope.deleteSubActivity = function () {
        if ($scope.selected.length > 0)
        {
            if (confirm("Are you sure you want to delete?"))
            {
                $scope.isLoading = true;
                var pParam = {'ids': $scope.selected};
                dataFactory.httpRequest('/admin/activity/deleteSubActivity', 'POST', '', pParam).then(function (response) {
                    $scope.isLoading = false;
                    var respType = (response.status == 200) ? "success" : "error";
                    $rootScope.showMsg(respType, response.message, "#formRow");
                    $rootScope.selected = [];
    ...
    $scope.editActivity = function (myData)
    {
    ...
    $scope.msg = "";
    $scope.addEditActivity = function (form)
    {
        ...
        if (form.validate())
        {
            ...
            $http({
                method: 'POST',
                url: env.get("apiroot") + '/admin/activity/addEditActivity',
                headers: {
...
//            dataFactory.httpRequest("/admin/activity/addEditActivity", "POST", {}, postData).then(function (response) {
//                console.log("respangular.element("input[type='file']").val(null);$scope.myFile = "";$scope.myFile = "";onse", response);
//                var respType = (response.status == 200) ? "success" : "error";
//                $rootScope.showMsg(respType, response.message, "#formRow2");
//                $scope.cancelShow = false;
//                $scope.btnName = "Save";
...
//            });
    ...
    $scope.addEditSubActivity = function (form)
    {
        ...
        if (form.validate())
        {
            ...
            $http({
                method: 'POST',
                url: env.get("apiroot") + '/admin/activity/addEditSubActivity',
                headers: {
                    'Content-Type': undefined
                },
                data: postData,
