angular
        .module('myapp').controller('AdminController', function (env,$scope, $rootScope, $http, $state, $location, dataFactory, localStorageService) {
    console.log("url", $state.current.url);
    $scope.pools = [];
    $scope.logOut = function ()
    {
        dataFactory.httpRequest('/admin/auth/logout', 'POST').then(function (response)
        {
    ...
    if ($state.current.url == "/profile")
    {
        ...
        var id = localStorageService.get("adminId");
        var postData = {"id": id};
        dataFactory.httpRequest("/admin/auth/getProfile", "POST", {}, postData).then(function (response) {
            $scope.profileData = response.responseData;
            $scope.imgReq = ($scope.profileData.image != '') ? false : true;
        });
        }
        $scope.getProfile();
    ...
    if ($state.current.url == "/social-links")
    {
        $scope.socialData = {"facebook": "", "twitter": '', "linkedin": "", "googleplus": ""};
        $scope.getSocial = function () {
            var postData = {};
            dataFactory.httpRequest("/admin/auth/getSocial", "POST", {}, postData).then(function (response) {
                $scope.socialData = response.responseData;
                console.log("social", $scope.socialData);
            });
        ...
        $scope.msg = "";
        $scope.updateSocialLink = function (form)
        {

            if (form.validate())
            {

                var postData = $scope.socialData;
                console.log("postData", postData);
                $scope.isLoading = true;
                dataFactory.httpRequest("/admin/auth/updateSocial", "POST", {}, postData).then(function (response) {
                    console.log("response", response);
                    $scope.isLoading = false;
                    $scope.socialData = response.responseData;
                    var respType = (response.status == 200) ? "success" : "error";
    ...
    if ($state.current.url == "/change-password")
    {
        $scope.cpData = {};
        var id = localStorageService.get("adminId");
        ...
        $scope.updatePassword = function (form)
        {
            ...
            if (form.validate())
            {
                ...
                $scope.isLoading = true;
                dataFactory.httpRequest("/admin/auth/changePassword", "POST", {}, postData).then(function (response) {
                    console.log("response", response);
                    $scope.isLoading = false;
                    var respType = (response.status == 200) ? "success" : "error";
                    $rootScope.showMsg(respType, response.message, "#formRow");
