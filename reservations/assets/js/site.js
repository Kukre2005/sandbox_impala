var loader = "<div align='center'><img src='" + site_url() + "/assets/loader.gif' id='loaderImg' class='loaderImg'/></div>";

function msgDiv(a, b, c) {
    if (b != "" && a != "") return '<div class="alert alert-' + (a == 200 ? "success" : "danger") + '" id="my' + c + '" > <a href="#" class="close" data-dismiss="alert" aria-label="close" title="close">\u00d7</a> <strong>' + (a == 200 ? "Success" : "Error") + "! </strong>" + b + "</div>"
}
$(document).ready(function () {   
    ...
    if(recaptchaRes.length == 0) {
        ...
        var message = "Please complete the reCAPTCHA challenge!";
        $("#captcha-error").html(message).show();
        return false;
    } 
            ...
            $.ajax({
                ...
                success: function (b) {
                    ...
                    if(b.status == '200')
                    {
                        window.location.href = site_url() + '?type=booking';
                    }else{
                        b = msgDiv(b.status, b.message, a.id);
                        $(b).insertBefore("#" + a.id);
                        $("#" + a.id + "").find("button[type=submit]").prop("disabled", false);
                    }
    ...
    $("#contactForm").validate({
        ...
        messages: {
            ...
            "info[hotelId]": {
                required: "Please select your resort"
            },
            "info[adults]": {
                required: "Please select no of adults"
            },
            "info[kids]": {
                required: "Please select no of kids"
            },
            "info[service]": {
        ...
        submitHandler: function (a) {
            ...
            $.ajax({
                ...
                success: function (b) {
                    ...
                    b = msgDiv(b.status, (b.status == 200) ? 'Thanks for contacting us, our support team will get back to you shortly.' : b.message, a.id);
    ...
    $("#quoteForm").validate({
        ...
        messages: {
            ...
            "info[reservationDate]": {
                required: "Please enter reservation date"
            },
            "info[hotelId]": {
                required: "Please select your resort"
            },
    ...
    jQuery.validator.addMethod("checkPass", function () {
        ...
        console.log("$('input[name=info[type]).val()", $("input[name='info[type]").val());
        if ($("input[name='info[type]").val() == "private_transport") {
            a = a.split("-");
            var b = parseInt(a[0]);
            a = parseInt(a[1])
        } else b = a;
        ...
        if (c < b || c > a) {
            console.log("Total number of adults and kids must be greater than " + b + " and less then or equal to " + a + ".");
            return false
        }
        return true
    ...
    $("#privateTransForm").validate({
        ...
        rules: {
            ...
            "info[adults]": {
                required: function () {
                    if ($('input[name="info[type]"]:checked', '#privateTransForm').val() == "private_transport") return true;
                    return false
                },
                number: true,
                minlength: 1,
                maxlength: 2,
                checkPass: function () {
                    if ($('input[name="info[type]"]:checked', '#privateTransForm').val() == "private_transport") {
                        return true
                    }
                    return false
                }
    ...
    $('body').on('click', '.apply-coupon-code', function () {
        if ($("#arrival_date").val() != "" && $("#arrival_date").val() != undefined && $("#arrival_date").val() != null) {
            removeCost();
        }
        ...
        if (tripCost > 0) {
            ...
            $.ajax({
                ...
                data: "coupon_code=" + couponCode + "&transportationType=" + transportationType,
                type: "POST",
                cache: false,
                dataType: "json",
                success: function (d) {
                    if (d.type == 1) {
                        ...
                        if (d.coupon_type == 0) {
                            ...
                            let cd = (parseNumber(tripCost * parseNumber(d.coupon_amount)) / 100);
                            discount_amount = cd.toFixed(2);
                        }
                        localStorage.setItem("isCouponApplied", 1);
                        console.log('discount_amount', discount_amount);
                        let final_trip_cost = parseNumber(tripCost - discount_amount).toFixed(2);
                        ...
                        $('.coupon-success-msg').removeClass('hide-class').text('Coupon applied successfully (Discount amount $' + discount_amount + ')');
    ...
    $('body').on('click', '.remove-coupon-code', function () {
        ...
        final_trip_cost = parseFloat(parseFloat(coupon_discount) + parseFloat(tripCost)).toFixed(2);
        console.log('final_trip_cost', final_trip_cost);
        $('#cost').val(final_trip_cost);
        $('#amount').text(final_trip_cost);
        ...
        var a = $("#passangers_range").val(),
            b = $("#hotelId").val(),
            c = $("input[name='info[type]").val(),
            e = $("#trip_type").val(),
            f = $("#couponCode").val(),
            g = $("#arrival_date").val();
        if (b != "") {
            a = "type=" + c + "&passengers=" + a + "&hotelId=" + b + "&service=" + e + "&couponCode=" + f + "&arrivalDate=" + g;
            $.ajax({
...
function cancelBook(){
    ...
    $.ajax({
        ...
        url: site_url() + "/transportation/cancel",
        data: {},
        type: "POST",
        cache: false,
        dataType: "json",
...
function setHotels (){
    ...
    var c = $("input[name='info[type]']:checked").val();
    a = "type=" + c;
    if(c == 'cabo_shuttle')
    {
    $('#showAdult').hide();
    ...
    $.ajax({
        url: site_url() + "/transportation/getHotels",
        data: a,
        type: "POST",
        cache: false,
        dataType: "json",
        success: function (d) {
            $('#hotelId').html(d.hotelHtml);
            setCost();
        },
        error: function () {
            $("#amount").html("0");
...
function setCost() {
    ...
    if (isCouponApplied != "" && isCouponApplied != undefined && isCouponApplied != null && isCouponApplied == 1) {
        return false;
    }
    var 
        b = $("#hotelId").val(),
        c = $("input[name='info[type]']:checked").val(),
        e = $("#trip_type").val(),
        f = $("#couponCode").val(),
        g = $("#arrival_date").val();
        a = $("#passangers_range_"+c).val();
    
    if(c == 'cabo_shuttle')
    {
        $('#passangers_range_private_transport').hide();
    }else{
        $('#passangers_range_cabo_shuttle').hide();
    }   
    $("#passangers_range_"+c).show(); 
    ...
    if (b != "") {
        ...
        $.ajax({
            ...
            success: function (d) {
                console.log("D", d);
                let code_details = d.code_details;
                if(parseInt(d.cost) == 0){
                    $("#errorInfo").css({display: "block"});
                    let trip_type = $("#trip_type").val()
                    $("#errorInfo span").html(`<b>${trip_type == 'one_way'? "One Way Trip": "Round Trip"}</b>`)
                }else{
