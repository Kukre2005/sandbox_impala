...

{
...

    {
        ...
        $data['loginData'] = array();

        $code = "";

        $type = "";

        $data['disableCabo'] = $data['disablePrivate'] = "";
        ...
        if ($this->session->userdata("loginData")) {
            ...
            $dbTypeP = $this->Commonmodel->get_single_data("hotelgroups", "name = '$code' and type = '$type'");



            $dbTypeC = $this->Commonmodel->get_single_data("hotelgroups", "name = '$code' and type = 'cabo_shuttle'");

            

            if(gettype($dbTypeP) == 'array' && gettype($dbTypeC) == 'array'){
            ...
            $data["dbHotels"] = $this->Commonmodel->get_data("hotels", "status = '1' and (id in (select hotels from hotelgroups where name = '$code' and type = '$type') or find_in_set(id,(select hotels from hotelgroups where name = '$code' and type = '$type')))", "*", "name asc");
...

    {
        ...
        foreach ($data['dbHotels'] as $h) {

            $hotelHtml .= "<option value='" . $h['id'] . "'>" . $h['name'] . "</option>";

        }

        $data['hotelHtml'] = $hotelHtml;
...

    {
        ...
        if ($_POST && count((array)$_POST) > 0) {

            extract($_POST['info']);

            $c = 0;

        	$code = $_POST['info']['code'];
            ...
            if (!empty($code) && $email) {
                ...
                $dbHotel = $this->Commonmodel->get_single_data("hotelgroups", "name like '$code'", $fields = '*');

                $c = count((array)$dbHotel);

            }

            $rules['info[email]'] = array('email', 'required|valid_email');

            $rules['info[code]'] = array('code', 'required|callback_checkNotExists[' . $c . '|This code does not exists]');
            ...
            if ($this->form_validation->run() == true) {
                ...
                // $info['hotelId'] = $dbHotel['id'];

                $info['code'] = $code;

                $info['passengers'] = "1-5";

                $this->session->set_userdata("loginData", $info);

               // print_r($this->session->userdata("loginData"));

                $message = "Your details are successfully submitted";
...

    {
        ...
        $data["dbHotels"] = $this->Commonmodel->get_data("hotels", "status = '1' and FIND_IN_SET(`id`,(select group_concat(hotels SEPARATOR ',') from hotelgroups where type = 'cabo_shuttle'))", "*", "name asc");
...

    {
        ...
        if ($_POST && count($_POST) > 0) {
            ...
            $rules['passengers'] = array('passengers', 'required');

            $rules['hotelId'] = array('hotel', 'required');

            $rules['service'] = array('type of service', 'required');
            ...
            if ($this->form_validation->run() == true) {
                ...
                if ($type == 'private_transport') {
                    ...
                    $field = "cost_" . $service . "_" . str_replace("-", "_", $passengers);
                ...
                $cost = $this->Commonmodel->get_table_field("hotelgroups", "find_in_set('$hotelId',hotels) != 0 and type = '$type'", $field);
                ...
                //                $dbCoup = $this->Commonmodel->get_single_data("coupon", "status = '1' and code = '$couponCode'", "*");
                ...
                if (isset($arrivalDate) && !empty($arrivalDate)) {
                    ...
                    $atime = date("H:i", strtotime($arrivalDates[1] . " " . $arrivalDates[2]));

                    $data['atime'] = $atime;

                    $day = date('l', strtotime($adate));

                    $dbArr = $this->Commonmodel->get_single_data("discount", "day = '$day' and  '$atime' between fromTime and toTime and type = '$type'", "discount");
...

    {
        ...
        // $YOUR_DOMAIN = 'http://reservations.impalacabo.com/payment-method';
        ...
        if ($_POST && count($_POST) > 0) {
            ...
            $u = ($type == 'private_transport') ? 'private-transportation' : 'cabo-shuttle';
            ...
            $rules['info[passengers]'] = array('passengers', 'required');
            ...
            if ($this->form_validation->run() == true) {
                ...
                if (isset($arrivalDate) && !empty($arrivalDate)) {
                    ...
                    if ($info['service'] == 'round_trip') {
                        ...
                        $dtime = date("H:i", strtotime($departureDates[1] . " " . $departureDates[2]));
                ...
                if (!empty($coupon_code)) {
                    ...
                    if (!empty($coupon_details) && strtotime(date('Y-m-d')) <= strtotime($coupon_details[0]['end_date'])) {

                        $coupon_type = $coupon_details[0]['coupon_type'];

                        $coupon_amount = $coupon_details[0]['coupon_amount'];

                        $info['festiveCouponCode'] = $coupon_code;

                        $festiveCouponDiscount = 0;

                        if ($coupon_type == 0) {

                            $festiveCouponDiscount = $coupon_amount;

                        } else {

                            $festiveCouponDiscount = round(($info['calculateAmt'] * $coupon_amount) / 100, 2);

                        }

                        $info['festiveCouponDiscount'] = $festiveCouponDiscount;

                        $info['finalAmount'] = $info['finalAmount'] - $festiveCouponDiscount;

                        $cost = $cost - $festiveCouponDiscount;
            ...
            //    echo $cost;exit;
                ...
                if ($cost != 0) {
                    ...
                    //$info['session_id'] = $_REQUEST['session_id'];
                    ...
                    $checkout_session = \Stripe\Checkout\Session::create([
                      ...
                      'line_items' => [[
                        ...
                        'price_data' => [
                          ...
                          'product_data' => [
                            ...
                            'images' => [base_url('assets/images/logo.jpg')],
                      ...
                      'success_url' => 'http://reservations.impalacabo.com/transportation/bookingPaymentSuccess',

                      'cancel_url' =>  'http://reservations.impalacabo.com/transportation/BookingCancelled',
        ...
        if ($mstatus == 'error') {
            ...
            $data['info']['hotelId'] = (isset($data['loginData']['hotelId'])) ? $data['loginData']['hotelId'] : '';
    ...
    public function bookingPaymentSuccess() { 
        ...
        $u = ($info['type'] == 'private_transport') ? 'private-transportation' : 'cabo-shuttle';
            ...
            try {
               ...
               // $this->Commonmodel->update_data("booking", $info, "id = '$id'");
                ...
                $hotelCode = $this->Commonmodel->get_table_field("hotels", "id = '" . $info['hotelId'] . "'", "code");

                $to = FROM_PAYPAL_EMAIL;

                //$to = 'iamranit81@gmail.com';

                $t = ($type == 'private_transport') ? "Private Transportation" : "Cabo Shuttle";

                $subject = "A user has made a booking with " . SITE_NAME . " for " . $t . ".";

                $msgBody = '<html><body>';

                $msgBody .= '<img src="' . site_url('assets/img/logo.jpg') . '" alt="Impala" />';

                $userMsgBody = $msgBody;

                $msgBody .= "<div><strong>Dear Admin,</strong><br/>A user - " . $info['name'] . " has made a booking for hotel - " . $hotelName . " for " . $t . " with us.Please check the following booking details</div>";

                $userMsgBody .= "<div><strong>Dear $name,</strong><br/>Your booking has been done successfully for hotel - " . $hotelName . " for " . $t . " with " . SITE_NAME . ".Please check the following booking details</div>";

                $tableBody  = '<p>You have successfully booked for Private Transaportation- Our confirmation system will send you an email from: <a href="mailto:no_reply@impalacabo.com">no_reply@impalacabo.com</a> with your confirmation transportation voucher with all details and map, please be aware that some Antivirus send our email to the Spam!</p><br/></br>';

                $tableBody .= '<table rules="all" style="border-color: #666;" cellpadding="10">';

                $tableBody .= "<tr><td><strong>Paypal Transaction Id:</strong> </td><td>"

                    . ""

                    . " " . strip_tags($info['transactionId']) . "</td></tr>";



                $tableBody .= "<tr><td><strong>Booking Date:</strong> </td><td>" . date("d/m/Y", strtotime($info['createdAt'])) . "</td></tr>";

                $tableBody .= "<tr style='background: #eee;'><td><strong>Name:</strong> </td><td>" . strip_tags($info['name']) . "</td></tr>";

                $tableBody .= "<tr><td><strong>Email:</strong> </td><td>" . strip_tags($info['email']) . "</td></tr>";

                $tableBody .= "<tr><td><strong>Phone:</strong> </td><td>" . strip_tags($info['phone']) . "</td></tr>";

                $tableBody .= "<tr><td><strong>Passengers:</strong> </td><td>" . strip_tags($info['passengers']) . "</td></tr>";

                $tableBody .= "<tr><td><strong>Hotel:</strong> </td><td>" . $hotelName . "</td></tr>";

                $tableBody .= "<tr><td><strong>Type Of Services:</strong> </td><td>" . $info['service'] . "</td></tr>";

                $tableBody .= "<tr><td><strong>Arrival Date:</strong> </td><td>" . strip_tags($arrivalDate) . "</td></tr>";

                $tableBody .= "<tr><td><strong>Arrival Flight Info: Airline, Flight Number and Time):</strong> </td><td>" . strip_tags($info['arrivalFlight']) . "</td></tr>";

                if ($service == 'round trip') {

                    $tableBody .= "<tr><td><strong>Departure Date:</strong> </td><td>" . strip_tags($departureDate) . "</td></tr>";

                    $tableBody .= "<tr><td><strong>Departure Flight Info: Airline, Flight Number and Time:</strong> </td><td>" . strip_tags($info['departureFlight']) . "</td></tr>";

                }

                if ($type == 'private_transport') {

                    $tableBody .= "<tr><td><strong>Adults:</strong> </td><td>" . strip_tags($adults) . "</td></tr>";

                    $tableBody .= "<tr><td><strong>Kids:</strong> </td><td>" . strip_tags($kids) . "</td></tr>";

                }



                $tableBody .= "<tr><td><strong>Comments:</strong> </td><td>" . htmlentities($info['comments']) . "</td></tr>";

                if (!empty($hotelCode)) {

                    $tableBody .= "<tr><td><strong>Coupon Code:</strong> </td><td>" . strip_tags($hotelCode) . "</td></tr>";

                }

                if (!empty($info['couponCode'])) {

                    $tableBody .= "<tr><td><strong>Coupon Code:</strong> </td><td>" . strip_tags($info['couponCode']) . "</td></tr>";

                    $tableBody .= "<tr><td><strong>Coupon Cost:</strong> </td><td>$" . strip_tags($info['couponAmount']) . "</td></tr>";

                }

                if (!empty($info['discount'])) {

                    $tableBody .= "<tr><td><strong>Discount:</strong> </td><td>" . strip_tags($info['discount']) . " %</td></tr>";

                    $tableBody .= "<tr><td><strong>Discount Cost:</strong> </td><td>$" . strip_tags($info['discountCost']) . " </td></tr>";

                }



                $tableBody .= "<tr><td><strong>Booking Amount:</strong> </td><td>$ " . strip_tags($info['finalAmount']) . "</td></tr>";

                $tableBody .= "<tr><td><strong>Booking Status:</strong> </td><td>" . strip_tags($info['bookStatus']) . "</td></tr>";

                if (!empty(@$info['festiveCouponCode'])) {

                    $tableBody .= "<tr><td><strong>Coupon Code:</strong> </td><td>" . strip_tags(@$info['festiveCouponCode']) . "</td></tr>";

                }

                $tableBody .= "<tr style='background: #eee;'><td><strong>Booking Id:</strong> </td><td>" . strip_tags($info['bookingId']) . "</td></tr>";

                $tableBody .= "</table><br>";

                $tableBody .= "<div>You will receive your confirmation transportation voucher from: no_reply@impalacabo.com</div>";

                $tableBody .= '<br><div>Regards,<br><br>';

                $tableBody .= "<strong>" . SITE_NAME . "</strong><br><br>";

                $tableBody .= '<img src="' . site_url('assets/img/logo.jpg') . '" alt="Impala" />';
                ...
                $o = $this->Email_model->sendEmail($to, $subject, $msgBody, ["http://reservations.impalacabo.com/assets/img/Terminal1.jpg", "http://reservations.impalacabo.com/assets/img/Terminal2.jpg"], FROM_EMAIL, FROM_NAME);



                $userTo = $info['email'];

                $usersubject = SITE_NAME . " - Your booking has been made successfully with $t.";



                $o = $this->Email_model->sendEmail($userTo, $usersubject, $userMsgBody, ["http://reservations.impalacabo.com/assets/img/Terminal1.jpg", "http://reservations.impalacabo.com/assets/img/Terminal2.jpg"], FROM_EMAIL, FROM_NAME);
            ...
            redirect('transportation/BookingComplete?bookingId=' . $bookId);
            ...
            //    echo $cost;exit;
                ...
                if ($cost != 0) {
                    ...
                    $SECFields = array(
                        ...
                        'returnurl' => site_url('transportation/GetBookingDetails'), // Required.  URL to which the customer will be returned after returning from PayPal.  2048 char max.

                        'cancelurl' => site_url('transportation/BookingCancelled'), // Required.  URL to which the customer will be returned if they cancel payment on PayPal's site.
                    ...
                    if (!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])) {
                        ...
                        $data['paypalMsg'] = "Some error occurred during processing with paypal";
                        ...
                        if ($o) {
                            ...
                            $PayPalResult['NewRedirectUrl'] = str_replace('continue', 'commit', $PayPalResult['REDIRECTURL']);
                            ...
                            $redu = str_replace('continue', 'commit', $PayPalResult['REDIRECTURL']);
...

    {
        ...
        if (!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])) {
            ...
            $paypalData['paypal_payer_id'] = isset($PayPalResult['PAYERID']) ? $PayPalResult['PAYERID'] : '';

            $paypalData['phone_number'] = isset($PayPalResult['PHONENUM']) ? $PayPalResult['PHONENUM'] : '';

            $paypalData['email'] = isset($PayPalResult['EMAIL']) ? $PayPalResult['EMAIL'] : '';

            $paypalData['first_name'] = isset($PayPalResult['FIRSTNAME']) ? $PayPalResult['FIRSTNAME'] : '';

            $paypalData['last_name'] = isset($PayPalResult['LASTNAME']) ? $PayPalResult['LASTNAME'] : '';



            foreach ($PayPalResult['PAYMENTS'] as $payment) {

                $paypalData['shipping_name'] = isset($payment['SHIPTONAME']) ? $payment['SHIPTONAME'] : '';

                $paypalData['shipping_street'] = isset($payment['SHIPTOSTREET']) ? $payment['SHIPTOSTREET'] : '';

                $paypalData['shipping_city'] = isset($payment['SHIPTOCITY']) ? $payment['SHIPTOCITY'] : '';

                $paypalData['shipping_state'] = isset($payment['SHIPTOSTATE']) ? $payment['SHIPTOSTATE'] : '';

                $paypalData['shipping_zip'] = isset($payment['SHIPTOZIP']) ? $payment['SHIPTOZIP'] : '';

                $paypalData['shipping_country_code'] = isset($payment['SHIPTOCOUNTRYCODE']) ? $payment['SHIPTOCOUNTRYCODE'] : '';

                $paypalData['shipping_country_name'] = isset($payment['SHIPTOCOUNTRYNAME']) ? $payment['SHIPTOCOUNTRYNAME'] : '';
...

    {
        ...
        $Payment = array(
            ...
            'shippingamt' => number_format($paypalData['PAYMENTREQUEST_0_SHIPPINGAMT'], 2), // Total shipping costs for this order.  If you specify SHIPPINGAMT you mut also specify a value for ITEMAMT.

            'handlingamt' => number_format($paypalData['PAYMENTREQUEST_0_HANDLINGAMT'], 2), // Total handling costs for this order.  If you specify HANDLINGAMT you mut also specify a value for ITEMAMT.
        ...
        if (!$this->paypal_pro->APICallSuccessful($PayPalResult['ACK'])) {
            ...
            foreach ($PayPalResult['PAYMENTS'] as $payment) {
                ...
                $paypalData['paypal_transaction_id'] = isset($payment['TRANSACTIONID']) ? $payment['TRANSACTIONID'] : '';

                $paypalData['paypal_fee'] = isset($payment['FEEAMT']) ? $payment['FEEAMT'] : '';
            ...
            $info['payerName'] = $paypalData['FIRSTNAME'] . " " . $paypalData['LASTNAME'];
            ...
            $info['sellerEmail'] = $paypalData['PAYMENTREQUEST_0_SELLERPAYPALACCOUNTID'];
...

    {
        ...
        $dbBook = $this->Commonmodel->get_single_data("booking", "bookingId = '$bookingId' and bookStatus = 'booked'", "*,(select name from hotels where hotels.id = booking.hotelId) as hotelName");
        ...
        if (count($dbBook) > 0) {
            ...
            $data['returnText'] = "Booking Page";

            $data['paypalCase'] = "booked";

            $data['transport_type'] = $data['type'] == 'cabo_shuttle' ? "Cabo Shuttle Transportation" : "Private Transaportation";

            $data['paypalMsg'] = "We have now reached the final thank you / receipt page and the payment has been processed!  We have added the PayPal transaction ID to the Billing Information, which was provided in the DoExpressCheckoutPayment response.";
...

    {
        ...
        $bookData['bookStatus'] = 'canceled';

        $bookData['payStatus'] = 'canceled';

        //echo "<pre>" . print_r($bookData, true) . "</pre>"; exit;

        //$o = $this->Commonmodel->save_data("booking", $bookData);
        ...
        $data['paypalTitle'] = "Payment Cancelled";
        ...
        $data['paypalMsg'] = "The payment has not been processed at this point because you cancelled the payment.";
