...
class Home extends MIK_Controller {
    ...
    public function subscribe() {
        /**
         * Added reCAPTCHA verification for subscription form
         * This code verifies the reCAPTCHA response before processing the subscription.
         * If the reCAPTCHA verification fails, it returns an error message.
         */
        if(isset($_POST['g-recaptcha-response']) && !empty($_POST['g-recaptcha-response'])){
            $secret = '6LcEYGAUAAAAACs4chvixfHOLrlc-kTsU7y8M30l';
            ...
            $data['status'] = 201;
            $data['message'] = "Please click on the reCAPTCHA box.";
            echo json_encode($data);
            return;
        }

        // Proceed with the subscription process if reCAPTCHA is valid
        $data = array();
        $status = 200;
        $message = "";
    ...
    public function saveContact() {
        ...
        //                    $message .= $o;
                    ...
                    $status = 201;
                    $message = "Google reCAPTCHA verification failed, please try agian.";
                    $data['status'] = $status;
                    $data['message'] = $message;
