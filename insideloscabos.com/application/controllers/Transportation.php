...
class Transportation extends MIK_Controller
{
  ...
  public function bookingPaymentSuccess()
  {
    ...
    try {
      ...
      $msgBody = "<html><body>";
      $msgBody .= '<img src="' . site_url("assets/img/newlogo.jpg") . '" alt="Insideloscabos" />';
      $userMsgBody = $msgBody;
      ...
      $userMsgBody .= "<div><strong>Dear " . $info["name"] . ",</strong><br/>Your booking has been done successfully for hotel - " . $hotelName . " for " . $t . " with " . SITE_NAME . ". Please check the following booking details</div>";
      
      // Start Table Body
      $tableBody = '<p>You have successfully booked for Private Transportation- Our confirmation system will send you an email from: <a href="mailto:no_reply@insideloscabos.com">no_reply@insideloscabos.com</a> with your confirmation transportation voucher with all details and map, please be aware that some Antivirus send our email to the Spam!</p><br/><br/>';
      $tableBody .= '<table rules="all" style="border-color: #666; border-collapse: collapse;" cellpadding="10">';
  
      // Add Rows to Table
      if (!empty($info["transactionId"])) {
          $tableBody .= "<tr><td><strong>Paypal Transaction Id:</strong> </td><td>" . strip_tags($info["transactionId"]) . "</td></tr>";
      }
      ...
      if (!empty($info["arrivalDate"])) {
          $tableBody .= "<tr><td><strong>Arrival Date:</strong> </td><td>" . strip_tags($info["arrivalDate"]) . "</td></tr>";
      }
  
      if (!empty($info["arrivalFlight"])) {
      ...
      if ($info["service"] == "round trip") {
          if (!empty($info["departureDate"])) {
              $tableBody .= "<tr><td><strong>Departure Date:</strong> </td><td>" . strip_tags($info["departureDate"]) . "</td></tr>";
          }
          if (!empty($info["departureFlight"])) {
      ...
      if ($type == "private_transport") {
          if (!empty($info["adults"])) {
              $tableBody .= "<tr><td><strong>Adults:</strong> </td><td>" . strip_tags($info["adults"]) . "</td></tr>";
          }
          if (!empty($info["kids"])) {
              $tableBody .= "<tr><td><strong>Kids:</strong> </td><td>" . strip_tags($info["kids"]) . "</td></tr>";
          }
      }
  ...
  function DoExpressCheckoutPayment()
  {
    ...
    if (!$this->paypal_pro->APICallSuccessful($PayPalResult["ACK"])) {
      ...
      try {
        ...
        $tableBody = '<p>You have successfully booked for Private Transportation. Our confirmation system will send you an email from: <a href="mailto:no_reply@insideloscabos.com">no_reply@insideloscabos.com</a> with your confirmation transportation voucher with all details and map. Please be aware that some antivirus programs send our email to the Spam folder!</p><br/></br>';
        $tableBody .= '<table rules="all" style="border-color: #666;" cellpadding="10">';
        ...
        if (!empty(@$info["festiveCouponCode"])) {
            $tableBody .= "<tr><td><strong>Festive Coupon Code:</strong> </td><td>" . strip_tags(@$info["festiveCouponCode"]) . "</td></tr>";
        }
