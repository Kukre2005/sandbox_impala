app.controller('GalleryController', function (env,$window, $scope, $rootScope, $http, $filter, $state, $stateParams, $location, dataFactory, localStorageService, constantData) {
    ...
    $scope.init = function ()
    {
        ...
        var u =  "galleryInfo";
        console.log("u", u);
        dataFactory.httpRequest('/admin/gallery/' + u, 'POST', '', postParam).then(function (response) {
    ...
    $scope.deleteGallery = function () {
        if ($scope.selected.length > 0)
        {
            if (confirm("Are you sure you want to delete?"))
            {
                ...
                dataFactory.httpRequest('/admin/gallery/deleteGallery', 'POST', '', pParam).then(function (response) {
                    $scope.isLoading = false;
    ...
    $scope.editGallery = function (myData)
    {
        ...
        if (myData)
        {
            ...
            cData.image = env.get("apiroot") + '/' + cData.thumbnail_image;
    ...
    $scope.addEditGallery = function (form)
    {
        ...
        if (form.validate())
        {
            ...
            $http({
                method: 'POST',
                url: env.get("apiroot") + '/admin/gallery/addEditGallery',
                headers: {
